...
- **v2.0d 개선(2026-07-15 확정)**:
  - **내 저장은 20곳 상한과 무관하게 항상 오버레이**: 뷰포트 검색 20곳과 별개로, 보이는 영역 안의 내 저장 장소는 무조건 핀으로 띄운다(place_id로 검색결과와 dedupe). 저장 시 **카테고리(버킷)를 저장**해 저장-전용 핀도 올바른 색/아이콘. 친구 저장도 동일 메커니즘이나 **친구 그래프(v2.1) 이후**.
  - **추적/탐색 모드 토글**: 내 위치 FAB를 상태 토글로 — 추적ON(차 따라감, 아이콘 강조) ↔ 탐색(자유 이동). 손대면 자동으로 탐색으로 바뀌고 FAB에 반영.
  - **정보카드**: 닫기(X 버튼 + 지도 탭) + 주소·전화·영업시간(영업중 여부) 표시. **평점/리뷰는 넣지 않음**(§2 공개평점 없음 원칙). 사진은 보류. 카드는 시스템 내비바 위로 띄우고, 정보만 스크롤·저장/길찾기 버튼은 고정 푸터. 핀 이름 라벨은 투명 배경+흰 헤일로.
  - **뷰포트 20 선정 = 중심 거리순(DISTANCE)**. POPULARITY(구글 저명도=리뷰·평점·인기 반영)는 조작 가능·공개평점 원칙과 충돌하므로 안 씀. 진짜 "눈에 띄는" 신호는 v2.1의 친구 저장 밀도로 구현.
